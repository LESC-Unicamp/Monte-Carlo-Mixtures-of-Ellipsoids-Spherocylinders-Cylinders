...
| Maximum attempts<br> (<code>RND</code> only) | <code>max_attempts_rnd</code> | Used to select the maximum number of attempts to randomly distribute particles inside a cubic box<br> (random configuration only) | Any non-zero, positive <code>INTEGER</code> number |
