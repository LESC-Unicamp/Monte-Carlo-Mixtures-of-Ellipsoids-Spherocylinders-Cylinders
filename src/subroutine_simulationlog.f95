...
IF( ConfigurationSelection(4) ) THEN
  ...
  WRITE( CharLabel(43,1), "(G0)"   ) nAdjustmentMovementRandomConfig
  WRITE( CharLabel(75,1), "(G0)"   ) nAdjustmentVolumeRandomConfig
...
IF( RNGeneratorLogical(1) ) THEN
  WRITE( CharLabel(76,1), "(G0)" ) "[FORTRAN]"
ELSE IF( RNGeneratorLogical(2) ) THEN
  WRITE( CharLabel(76,1), "(G0)" ) "[BITWISE]"
END IF
IF( FullPotentialTypeLogical(1) ) THEN
  WRITE( CharLabel(64,1), "(G0)" ) "[REFERENCE] Purely Repulsive Hard-Core"
ELSE IF( FullPotentialTypeLogical(2) ) THEN
  WRITE( CharLabel(64,1), "(G0)" ) "[REFERENCE] Square Well"
END IF
...
END DO
IF( RNGeneratorLogical(1) ) THEN
  WRITE( CharLabel(72,1), "('[',1X,3(G0,',',1X))"    ) InitialSeed(1:3)
  WRITE( CharLabel(77,1), "(4(G0,',',1X),G0,1X,']')" ) InitialSeed(4:8)
ELSE IF( RNGeneratorLogical(2) ) THEN
  WRITE( CharLabel(72,1), "('[',1X,3(G0,',',1X))" ) InitialSeed(1:3)
  WRITE( CharLabel(77,1), "(G0,',',1X,G0,1X,']')" ) InitialSeed(4:5)
END IF
...
StringsHeader(6) = "Supervisor: Luís Fernando Mercier Franco"
...
StringsText(45,1) = "Mov. Adjustment Freq. (Random Configuration): Every "//TRIM( CharLabel(43,1) )//" Cycle(s)"
StringsText(81,1) = "Vol. Adjustment Freq. (Random Configuration): Every "//TRIM( CharLabel(75,1) )//" Cycle(s)"
...
StringsText(48,1) = "Max. Isotropic Volume Scaling (Random Configuration): "//TRIM( CharLabel(46,1) )
StringsText(62,1) = "Max. Anisotropic Volume Scaling (Random Configuration): "//TRIM( CharLabel(57,1) )
...
StringsText(78,1) = "Initial Seed Values: "//TRIM( CharLabel(72,1) )
StringsText(83,1) = ""//TRIM( CharLabel(77,1) )
...
IF( .NOT. FileExist ) THEN
  OPEN( Unit= 95, File= "Simulation_Log.txt", Status= "NEW", Action= "WRITE" )
  ...
  &                                       CEILING( StringsSizeHeader(6) + 1 ) )//CH_VS
  WRITE( 95, "(G0)" ) CH_VL//REPEAT( CH_HS, 70 )//CH_VR
ELSE IF( FileExist ) THEN ! Append information
  OPEN( Unit= 95, File= "Simulation_Log.txt", Action= "WRITE", Status= "OLD", Position= "APPEND" )
...
IF( nComponents > 1 ) THEN
  ...
  DO cComponent = 1, nComponents
    ...
    &                   NINT( StringsSizeText(77,cComponent) ) )//CH_VS
...
ELSE IF( nComponents == 1 ) THEN
  ...
  &                   NINT( StringsSizeText(77,1) ) )//CH_VS
...
IF( ConfigurationSelection(4) ) THEN
  ...
  WRITE( 95, "(G0)" ) CH_VS//REPEAT( " ", 2 )//TRIM( StringsText(81,1) )//REPEAT( " ", NINT( StringsSizeText(81,1) ) - 1 )//CH_VS
...
WRITE( 95, "(G0)" ) CH_VS//REPEAT( " ", 1 )//TRIM( StringsText(82,1) )//REPEAT( " ", NINT( StringsSizeText(82,1) ) )//CH_VS
WRITE( 95, "(G0)" ) CH_VS//REPEAT( " ", 1 )//TRIM( StringsText(78,1) )//REPEAT( " ", NINT( StringsSizeText(78,1) ) )//CH_VS
WRITE( 95, "(G0)" ) CH_VS//REPEAT( " ", 1 )//TRIM( StringsText(83,1) )//REPEAT( " ", NINT( StringsSizeText(83,1) ) )//CH_VS
WRITE( 95, "(G0)" ) CH_VS//REPEAT( " ", 70 )//CH_VS
