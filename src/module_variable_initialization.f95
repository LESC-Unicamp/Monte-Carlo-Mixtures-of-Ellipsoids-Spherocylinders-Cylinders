...
! Adjustment frequency (random configuration) - Movement
READ( 10, * ) Dummy, nAdjustmentMovementRandomConfig
! Condition
IF( nAdjustmentMovementRandomConfig < 1 ) THEN
  WRITE( *, "(4G0)" ) "The movement adjustment frequency of the random configuration [", nAdjustmentMovementRandomConfig, "] ", &
  &                   "cannot be a negative integer nor zero. Exiting... "
  CALL Exit(  )
END IF

! Adjustment frequency (random configuration) - Volume change
READ( 10, * ) Dummy, nAdjustmentVolumeRandomConfig
! Condition
IF( nAdjustmentVolumeRandomConfig < 1 ) THEN
  WRITE( *, "(4G0)" ) "The volumetric adjustment frequency of the random configuration [", nAdjustmentVolumeRandomConfig, "] ", &
  &                   "cannot be a negative integer nor zero. Exiting... "
  CALL Exit(  )
END IF
...
READ( 10, * ) Dummy, RNGeneratorInquiry
...
! Potential type (perturbed system)
READ( 10, * ) Dummy, PerturbedPotentialType
CALL ToUpper( PerturbedPotentialType, LEN_TRIM( PerturbedPotentialType ), PerturbedPotentialType )
! Transforms characters into logical variables
PerturbedPotentialTypeLogical = .FALSE.
IF( PerturbedPotentialType == "HARDCORE" ) THEN
...
! Potential type (reference system)
READ( 10, * ) Dummy, FullPotentialType
CALL ToUpper( FullPotentialType, LEN_TRIM( FullPotentialType ), FullPotentialType )
! Transforms characters into logical variables
FullPotentialTypeLogical = .FALSE.
IF( FullPotentialType == "HARDCORE" ) THEN
...
END IF
! Stop condition 2
IF( .NOT. PerturbedPotentialTypeLogical(1) .AND. .NOT. FullPotentialTypeLogical(1) ) THEN
  WRITE( *, "(4G0)" ) "The perturbed potential [", TRIM( PerturbedPotentialType ), "] can only be used here if the reference ", &
  &                   " system is governed by a purely repulsive hard-core potential. Exiting..."
  CALL Exit(  )
END IF
...
! Potential inquiry
IF( .NOT. PerturbedPotentialTypeLogical(1) .OR. .NOT. FullPotentialTypeLogical(1) ) THEN
...
pLoop: DO
  IF ( ConfigurationSelection(1) ) THEN
    ...
    IF ( DABS( CubicRootFloat - DBLE( DNINT( CubicRootFloat ) ) ) <= 1.D-10 ) THEN
      EXIT pLoop
    ELSE
      WRITE( *, "(G0)" ) " "
...
IF( ConfigurationSelection(4) ) THEN
  ...
  WRITE( *, "(G0)" ) " "
  WRITE( *, "(G0,G0,G0)" ) "Movement Adjustment Frequency (Random Configuration): Every ", nAdjustmentMovementRandomConfig, &
  &                        " Cycle(s)"
  WRITE( *, "(G0,G0,G0)" ) "Volumetric Adjustment Frequency (Random Configuration): Every ", nAdjustmentVolumeRandomConfig, &
  &                        " Cycle(s)"
  WRITE( *, "(G0)" ) " "
...
IF( FullPotentialTypeLogical(1) ) THEN
  IF( PerturbedPotentialTypeLogical(1) ) THEN
    ...
    WRITE( *, "(G0)" ) "Perturbed Potential Type: [HARDCORE]"
...
WRITE( 105, "(G0,5(G0,1X))" ) "Perturbed_Potential_Type_Logical: ", PerturbedPotentialTypeLogical
WRITE( 105, "(G0,5(G0,1X))" ) "Full_Potential_Type_Logical: ", FullPotentialTypeLogical
...
WRITE( 105, "(2G0)" ) "Volumetric_Adjustment_Frequency_Random_Config: ", nAdjustmentVolumeRandomConfig
...
WRITE( 105, "(2G0)" ) "RNGenerator_Inquiry: ", "'"//TRIM( RNGeneratorInquiry )//"'"
WRITE( 105, "(2G0)" ) "Fixed_Seed_Logical: ", FixedSeedLogical
DescriptorBackupString = "(G0,2(G0,1X))"
WRITE( 105, DescriptorBackupString ) "Random_Number_Generator_Logical: ", RNGeneratorLogical
WRITE( 105, "(2G0)" ) "Backup_File_Inquiry: ", "'"//TRIM( BackupFileInquiry )//"'"
WRITE( 105, "(2G0)" ) "Backup_File_Logical: ", BackupFileLogical
WRITE( 105, "(2G0)" ) "Perturbed_Potential_Type: ", "'"//TRIM( PerturbedPotentialType )//"'"
WRITE( 105, "(2G0)" ) "Full_Potential_Type: ", "'"//TRIM( FullPotentialType )//"'"
IF( PerturbedPotentialTypeLogical(2) .OR. FullPotentialTypeLogical(2) ) THEN
...
WRITE( DescriptorBackupString, "(G0)" ) SeedSize
DescriptorBackupString = "(G0,"//TRIM( DescriptorBackupString )//"(G0,1X)"//")"
WRITE( 105, DescriptorBackupString ) "Initial_Seed: ", InitialSeed
...
READ( 105, * ) Dummy, PerturbedPotentialTypeLogical
READ( 105, * ) Dummy, FullPotentialTypeLogical
! Allocation
...
IF( CellListLogical .AND. ( PerturbedPotentialTypeLogical(2) .OR. FullPotentialTypeLogical(2) ) ) &
&   ALLOCATE( pCellListPotential(nParticles), pCellIndexPotential(3,nParticles) )
IF( PerturbedPotentialTypeLogical(2) .OR. FullPotentialTypeLogical(2) ) THEN
...
READ( 105, * ) Dummy, nAdjustmentVolumeRandomConfig
...
READ( 105, * ) Dummy, RNGeneratorInquiry
READ( 105, * ) Dummy, FixedSeedLogical
READ( 105, * ) Dummy, RNGeneratorLogical
! Allocation
IF( RNGeneratorLogical(1) ) THEN ! Fortran
  CALL RANDOM_SEED( Size= SeedSize )
  ALLOCATE( SeedValue(SeedSize), InitialSeed(SeedSize) )
ELSE IF( RNGeneratorLogical(2) ) THEN ! Bitwise
  SeedSize = 5
  ALLOCATE( SeedValue(SeedSize), InitialSeed(SeedSize) )
END IF
...
READ( 105, * ) Dummy, PerturbedPotentialType
READ( 105, * ) Dummy, FullPotentialType
IF( PerturbedPotentialTypeLogical(2) .OR. FullPotentialTypeLogical(2) ) THEN
...
IF( RNGeneratorLogical(1) ) CALL RANDOM_SEED( Put= INT( InitialSeed ) )
