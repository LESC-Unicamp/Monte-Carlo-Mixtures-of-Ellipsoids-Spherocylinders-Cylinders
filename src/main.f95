...
REAL( Kind= Real64 )                   :: EnergyChange                             ! Configurational energy criterion (reduced)
...
IF( .NOT. RestoreBackupFileLogical ) THEN
  ...
  IF( FixedSeedLogical ) THEN
    IF( RNGeneratorLogical(1) ) THEN ! Fortran
      ...
      ! Add fixed seed to random number generator
      CALL RANDOM_SEED( Put= INT( SeedValue ) )
    ELSE IF( RNGeneratorLogical(2) ) THEN ! Bitwise
      SeedSize = 5
      ALLOCATE( SeedValue(SeedSize), InitialSeed(SeedSize) )
      ! Fixed seed
      SeedValue(1) = 153587801
      SeedValue(2) = -759022222
      SeedValue(3) = 1288503317
      SeedValue(4) = -1718083407
      SeedValue(5) = -123456789
      InitialSeed  = SeedValue
    END IF
  ELSE IF( .NOT. FixedSeedLogical ) THEN
    IF( RNGeneratorLogical(1) ) THEN ! Fortran
      ...
      CALL RANDOM_SEED( Size= SeedSize )
      ALLOCATE( RandomSeed(SeedSize), SeedValue(SeedSize), InitialSeed(SeedSize) )
      CALL RANDOM_SEED( Get= RandomSeed )
      ! Attribute seed
      SeedValue   = RandomSeed
      InitialSeed = SeedValue
      ! Add random seed to random number generator
      CALL RANDOM_SEED( Put= INT( SeedValue ) )
    ELSE IF( RNGeneratorLogical(2) ) THEN ! Bitwise
      SeedSize = 5
      ALLOCATE( SeedValue(SeedSize), InitialSeed(SeedSize) )
      ...
      DO sSeed = 1, 5
        ...
        InitialSeed(sSeed) = SeedValue(sSeed)
      END DO
    END IF
  END IF
...
IF( .NOT. RestoreBackupFileLogical ) THEN
  IF( PerturbedPotentialTypeLogical(2) .OR. FullPotentialTypeLogical(2) ) THEN
    ...
    IF( .NOT. CellListControlPotential ) THEN
      ! Whole system
      CALL ComputeTotalPotentialEnergyInitialConfig(  )
    ELSE
      ! Linked lists
      CALL ListComputeTotalPotentialEnergyInitialConfig( BoxLength, BoxLengthInverse, HalfNeighboursControl )
    END IF
    ! Status
    CALL Sleep( 1 )
    WRITE( *, "(G0)", Advance= "YES" ) " Done!"
...
ELSE
  ...
  ! Recalculate packing fraction and number density
  PackingFraction    = ( TotalParticleVolume / BoxVolumeMC )
...
! Simulation cycles                                                                               !
! *********************************************************************************************** !
!  A 'cycle' is characterized by a trial move (rotation or translation) of a random particle or   !
...
DO iCycle = FirstCycle + 1, MaxSimulationCycles
  ...
  IF( RandomNumber <= MovementProbability .OR. EnsembleMC == "NVT" ) THEN
    ...
    ! Compute intermolecular potential if the system is not ruled by a purely repulsive hard-core potential
    IF( .NOT. FullPotentialTypeLogical(1) ) THEN
      ...
      ! Energy difference
      EnergyChange = iPotentialEnergyDifference(1) / ReducedTemperature
      ! Pseudorandom number generator (uniform distribution)
      IF( RNGeneratorLogical(1) ) CALL Random_Number( RandomNumber )
      IF( RNGeneratorLogical(2) ) CALL RandomNumberGenBitwise(  )
    END IF

    ! Configurational energy difference criterion
    IF( FullPotentialTypeLogical(1) .OR. DEXP( - EnergyChange ) >= RandomNumber ) THEN
  ...
  ELSE IF( RandomNumber > MovementProbability .AND. EnsembleMC == "NPT" ) THEN
    ...
    ! Assignment of previous potential energy
    IF( .NOT. FullPotentialTypeLogical(1) ) OldPotentialEnergy = TotalPotentialEnergyMC(1) 
    ...
    IF( .NOT. CheckBoxDistortion ) THEN
      ...
      IF( DEXP( - EnthalpyChange ) >= RandomNumber ) THEN
        ...
        IF( .NOT. Overlap ) THEN
          ...
          IF( .NOT. FullPotentialTypeLogical(1) ) THEN
            TotalPotentialEnergyMC = NewPotentialEnergy
          END IF
          ! Displacement counter update
          IF( MovementIsoVolumeChangeLogical ) THEN
  ...
  IF( iCycle <= nEquilibrationCycles ) THEN ! During equilibration only
    ...
    IF( MOD( iCycle, nAdjustmentMovementFrequency ) == 0 ) THEN
    ...
    IF( MOD( iCycle, nAdjustmentVolumeFrequency ) == 0 ) THEN
  ...
  IF( BackupFileLogical .AND. (MOD( iCycle, nSavingFrequency ) == 0 .OR. iCycle == FirstCycle + 1) ) THEN
    ...
    IF( CellListLogical ) THEN
      ...
      IF( CellListControl ) WRITE( 105, DescriptorBackupString ) "Cell_Head: ", pCellHead
      ...
      IF( CellListControl ) DescriptorBackupString = "(G0,"//TRIM( DescriptorBackupString )//"(G0,1X)"//")"
      IF( CellListControl ) WRITE( 105, DescriptorBackupString ) "Cell_List: ", pCellList
    ...
    END IF
    WRITE( 105, "(G0,9(G0,1X))" ) "Box_Length: ", BoxLengthMC
    WRITE( 105, "(G0,9(G0,1X))" ) "Box_Length_Inverse: ", BoxLengthInverseMC
    WRITE( 105, "(G0,G0)" ) "Box_Volume: ", BoxVolumeMC
    WRITE( 105, "(G0,G0)" ) "Box_Distortion: ", BoxDistortionMC
    ...
    IF( RNGeneratorLogical(1) ) WRITE( 105, "(G0,8(G0,1X))" ) "Seed_Value: ", SeedValue
    IF( RNGeneratorLogical(2) ) WRITE( 105, "(G0,5(G0,1X))" ) "Seed_Value: ", SeedValue
    CLOSE( 105 )
  END IF
...
IF( PerturbationCoefficientLogical .AND. EnsembleMC == "NVT" .AND. PerturbedPotentialTypeLogical(2) ) THEN
  ...
  IF( .NOT. FlagLogical .AND. ( MinBlocks <= MaxBlocks ) ) THEN
    DO rRange = 1, nRange
      ...
      WRITE( DescriptorString, "(G0)" ) nComponents
