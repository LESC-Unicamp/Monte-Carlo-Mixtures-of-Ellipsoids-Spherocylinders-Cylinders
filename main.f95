...
INTEGER*8 :: CI, CJ, C ! Counters (component)
INTEGER*8 :: MATRIX_C  ! Random matrix component
INTEGER*8 :: CYCLES    ! Cycles
INTEGER*8 :: NACCT     ! Move acceptance counter: Translation
INTEGER*8 :: NACCR     ! Move acceptance counter: Rotation
INTEGER*8 :: NACCV     ! Move acceptance counter: Volume change
INTEGER*8 :: MOVT      ! Move counter (Translation)
INTEGER*8 :: MOVR      ! Move counter (Rotation)
INTEGER*8 :: MOVV      ! Move counter (Volume change)
...
REAL*8                   :: BOXVM, BOXVN     ! Volume of simulation box (before/after a trial move)
REAL*8                   :: HNM              ! Enthalpy criterion (reduced)
REAL*8                   :: SCALE_FACTOR     ! Scaling factor
...
REAL*8, DIMENSION( 9 )   :: BOXLM, BOXLN     ! Length of simulation box (before/after a trial move)
REAL*8, DIMENSION( 9 )   :: BOXLM_I, BOXLN_I ! Length of simulation box (before/after a trial move)
REAL*8, DIMENSION( 9 )   :: BOXLMC           ! Length (x,y,z) of the simulation box
REAL*8, DIMENSION( 9 )   :: BOXLMC_I         ! Length (x,y,z) of simulation box (inverse)
...
! *********************************************************************************************** !
REAL*8, DIMENSION( : ), ALLOCATABLE    :: CUTOFF ! Cutoff distance
REAL*8, DIMENSION( : ), ALLOCATABLE    :: STRSH  ! String size
REAL*8, DIMENSION( : ), ALLOCATABLE    :: STRSS  ! String size
REAL*8, DIMENSION( :, : ), ALLOCATABLE :: STRST  ! String size
...
IF( TRAJ_CHECK ) THEN
  ...
  IF( GEOM_SELEC(1) ) THEN
    DO C = 1, COMPONENTS
      DO I = SUM( N_COMPONENT(0:(C-1)) ) + 1, SUM( N_COMPONENT(0:C) )
        WRITE( 20, * ) INDEX_P(C), R(1,I), R(2,I), R(3,I), Q(0,I), Q(1,I), Q(2,I), Q(3,I), 0.5D0 * DIAMETER(C), &
...
DO CYCLES = 1, MAX_CYCLES
  ...
  IF( CYCLES <= N_EQUIL ) THEN  ! During equilibration only
    ...
    IF( MOD( MOVV, N_ADJUST ) == 0 .AND. MOVV > 0 ) THEN
      ...
      IF( MC_ENSEMBLE == "NPT" ) THEN
        WRITE( 50, * ) CYCLES, RATIO, DVMAX, R_ACC_V, VOL_TYPE
        FLUSH( 50 )
      END IF
      ! Reset counter
      NACCV = 0
      ...
      IF( MC_ENSEMBLE == "NPT" ) THEN
        WRITE( 55, * ) CYCLES, BOXVMC, BOXLMC(1), BOXLMC(2), BOXLMC(3), BOXLMC(4), BOXLMC(5), BOXLMC(6), BOXLMC(7), BOXLMC(8), &
        &              BOXLMC(9)
        FLUSH( 55 )
      END IF
  ...
  IF( TRAJ_CHECK ) THEN
    IF( MOD ( CYCLES, N_SAVE ) == 0 ) THEN
      ...
      IF( GEOM_SELEC(1) ) THEN
        DO C = 1, COMPONENTS
          DO I = SUM( N_COMPONENT(0:(C-1)) ) + 1, SUM( N_COMPONENT(0:C) )
            WRITE( 20, * ) INDEX_P(C), RMC(1,I), RMC(2,I), RMC(3,I), QMC(0,I), QMC(1,I), QMC(2,I), QMC(3,I), &
  ...
  IF( MOD( CYCLES, N_SAVE ) == 0 .AND. MC_ENSEMBLE == "NPT" ) THEN
    WRITE( 70, * ) CYCLES, PACKING_F, TOTAL_RHO, BOXVMC, PRESS
    FLUSH( 70 )
  END IF
